import React from 'react';
import '../index.css';

const AboutPlayersPage = () => {
  return (
    <div className="page-content2">
      <h1 className="truegold1">About Players</h1>
      <section>
      <div className="section-container">
    <h2 className="azure-blue">Player Profiles 2024 Season</h2>
  </div>
        <div className="player-profiles">
          <div className="player-profile">
            <h3>Kevin Mier</h3>
            <p>Position: Goalkeeper</p>
            <p>Jersey Number: 1</p>
            <p>Appearences:56 </p>
            <p>Goals:0 </p>
            <p>Assists:1 </p>
            <p>Clean Sheets:20 </p>
          </div>
          <div className="player-profile2">
            <h3>Paolo Reyna</h3>
            <p>Position: Leftback/Left Midfielder</p>
            <p>Jersey Number: 2</p>
            <p>Appearences: 58 </p>
            <p>Goals: 2 </p>
            <p>Assists: 3 </p>
            <p>Clean Sheets: 21</p>
          </div>
          <div className="player-profile">
            <h3>Guillermo Marín</h3>
            <p>Position: Centreback/Rightback</p>
            <p>Jersey Number: 3</p>
            <p>Appearences: 54 </p>
            <p>Goals: 0</p>
            <p>Assists: 0 </p>
            <p>Clean Sheets: 21</p>
          </div>
          <div className="player-profile2">
            <h3>Brooks</h3>
            <p>Position: Centreback</p>
            <p>Jersey Number: 4</p>
            <p>Appearences: 22</p>
            <p>Goals: 0</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 0</p>
          </div>
          <div className="player-profile">
            <h3>Sergi Roberto</h3>
            <p>Position: Rightback</p>
            <p>Jersey Number: 5</p>
            <p>Appearences: 57</p>
            <p>Goals: 2</p>
            <p>Assists: 7</p>
            <p>Clean Sheets: 21</p>
          </div>
          <div className="player-profile2">
            <h3>Hugo Figueroa</h3>
            <p>Position: Centreback</p>
            <p>Jersey Number: 6</p>
            <p>Appearences: 20</p>
            <p>Goals: 1</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 1</p>
          </div>
          <div className="player-profile">
            <h3>Victor Ramos </h3>
            <p>Position: AM</p>
            <p>Jersey Number: 7</p>
            <p>Appearences: 28 </p>
            <p>Goals: 4</p>
            <p>Assists: 4</p>
            <p>Clean Sheets: 2</p>
          </div>
          <div className="player-profile2">
            <h3>Mateo Peralta</h3>
            <p>Position: LM</p>
            <p>Jersey Number: 8</p>
            <p>Appearences: 0</p>
            <p>Goals: 0</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 0</p>
          </div>
          <div className="player-profile">
            <h3>Alfonso Guerra</h3>
            <p>Position: Striker</p>
            <p>Jersey Number: 9</p>
            <p>Appearences: 0</p>
            <p>Goals: 0</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 0</p>
          </div>
          <div className="player-profile2">
            <h3>Bruno Ferreira</h3>
            <p>Position: Striker</p>
            <p>Jersey Number: 10</p>
            <p>Appearences: 12</p>
            <p>Goals: 2</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 1</p>
          </div>
          <div className="player-profile">
            <h3>Daniel Rojas</h3>
            <p>Position: Goalkeeper</p>
            <p>Jersey Number: 12</p>
            <p>Appearences: 2</p>
            <p>Goals: 0</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 2</p>
          </div>
          <div className="player-profile2">
            <h3>Javier Iglesias</h3>
            <p>Position: CM</p>
            <p>Jersey Number: 13</p>
            <p>Appearences: 16</p>
            <p>Goals: 0</p>
            <p>Assists:2</p>
            <p>Clean Sheets: 1</p>
          </div>
          <div className="player-profile">
            <h3>Federico Pereira</h3>
            <p>Position: CB/RB </p>
            <p>Jersey Number: 14</p>
            <p>Appearences: 54</p>
            <p>Goals: 0</p>
            <p>Assists: 0</p>
            <p>Clean Sheets: 20</p>
          </div>
          <div className="player-profile2">
            <h3>Carlos Alcarez</h3>
            <p>Position: CF</p>
            <p>Jersey Number: 15</p>
            <p>Appearences:23 </p>
            <p>Goals: 16</p>
            <p>Assists: 7</p>
            <p>Clean Sheets: 6</p>
          </div>
          <div className="player-profile">
            <h3>Franjo Marić</h3>
            <p>Position: CM</p>
            <p>Jersey Number: 16</p>
            <p>Appearences: 51 </p>
            <p>Goals: 6 </p>
            <p>Assists: 9 </p>
            <p>Clean Sheets: 8 </p>
          </div>
          <div className="player-profile2">
            <h3>Alexandro Bernabai</h3>
            <p>Position: Leftback</p>
            <p>Jersey Number: 17</p>
            <p>Appearences: 7 </p>
            <p>Goals: 0 </p>
            <p>Assists: 0 </p>
            <p>Clean Sheets: 1 </p>
          </div>
          <div className="player-profile">
            <h3>Rafa Roman</h3>
            <p>Position: Goalkeeper</p>
            <p>Jersey Number: 18</p>
            <p>Appearences: 0 </p>
            <p>Goals: 0 </p>
            <p>Assists: 0 </p>
            <p>Clean Sheets: 0 </p>
          </div>
          <div className="player-profile2">
            <h3>Raùl Reyes</h3>
            <p>Position: LW/CM/RW/AM</p>
            <p>Jersey Number: 19</p>
            <p>Injured</p>
            <p>Appearences: 2 </p>
            <p>Goals: 0 </p>
            <p>Assists: 1 </p>
            <p>Clean Sheets: 0 </p>
          </div>
          <div className="player-profile">
            <h3>Santiago Barrera</h3>
            <p>Position: Striker</p>
            <p>Jersey Number: 20</p>
            <p>Appearences: 0 </p>
            <p>Goals: 0 </p>
            <p>Assists: 0 </p>
            <p>Clean Sheets: 0 </p>
          </div>
          <div className="player-profile2">
            <h3>Leonal Duarte</h3>
            <p>Position: CF/AM/ST</p>
            <p>Jersey Number: 21</p>
            <p>Appearences: 58 </p>
            <p>Goals: 47 </p>
            <p>Assists: 20 </p>
            <p>Clean Sheets: 20 </p>
          </div>
          <div className="player-profile">
            <h3>Andreas Rey</h3>
            <p>Position: CF</p>
            <p>Jersey Number: 22</p>
            <p>Appearences: 0 </p>
            <p>Goals: 0 </p>
            <p>Assists: 0 </p>
            <p>Clean Sheets: 0 </p>
          </div>
          <div className="player-profile2">
            <h3>Yago Pardo</h3>
            <p>Position: ST/CF</p>
            <p>Jersey Number: 24</p>
            <p>Appearences: 55 </p>
            <p>Goals: 51 </p>
            <p>Assists: 22 </p>
            <p>Clean Sheets: 17 </p>
          </div>
          <div className="player-profile">
            <h3>Cristian Medina</h3>
            <p>Position: CM</p>
            <p>Jersey Number: 25</p>
            <p>Appearences: 51 </p>
            <p>Goals: 3 </p>
            <p>Assists: 7 </p>
            <p>Clean Sheets: 18 </p>
          </div>
          <div className="player-profile2">
            <h3>Denis Bouanga</h3>
            <p>Position: CF</p>
            <p>Jersey Number: 30</p>
            <p>Appearences: 16 </p>
            <p>Goals: 2 </p>
            <p>Assists: 2 </p>
            <p>Clean Sheets: 2 </p>
          </div>
          <div className="player-profile">
            <h3>Fabrizio Diaz</h3>
            <p>Position: CM</p>
            <p>Jersey Number: 31</p>
            <p>Appearences: 56 </p>
            <p>Goals: 7 </p>
            <p>Assists: 8 </p>
            <p>Clean Sheets: 20 </p>
          </div>
          <div className="player-profile2">
            <h3>Rojas</h3>
            <p>Position: RB</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Cornejo</h3>
            <p>Position: CF</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Carlos Morales</h3>
            <p>Position: CM</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Blanco</h3>
            <p>Position: LW</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Cabrera</h3>
            <p>Position: LW</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Javi Castro</h3>
            <p>Position: cdm</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Luis Vega</h3>
            <p>Position: lm</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Iker Castillo</h3>
            <p>Position: Leftback</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Vincente Rivero</h3>
            <p>Position: CM</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Pedro Peralta</h3>
            <p>Position: CM</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Rafael Castro</h3>
            <p>Position: Striker</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Julen Navarro</h3>
            <p>Position: RW</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Guillermo Castro</h3>
            <p>Position: RWB</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile">
            <h3>Manuel Salazar</h3>
            <p>Position: Striker</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
          <div className="player-profile2">
            <h3>Ginella</h3>
            <p>Position: CM</p>
            <p>LOAN</p>
            <p>Jersey Number: N/A</p>
            <p>Appearences: N/A </p>
            <p>Goals: N/A </p>
            <p>Assists: N/A </p>
            <p>Clean Sheets: N/A </p>
          </div>
        </div>
      </section>
      <section className='leaguestats'>
        <h2 className="azure-blue">Team Statistics 2025 League season</h2>
        <p>Matches Played: 27</p>
        <p>Goals Scored: 68</p>
        <p>Assists: 32</p>
        <p>Top Scorer: Pardo, 20 Goals</p>
        <p>Top Assister: Duarte, Pardo, 8 Assists</p>
        <p>League POTY: Pardo</p>
        <p>Club POTY: Pardo</p>
        <p>YPOTY:Pardo</p>
        <p>League Champions</p>
      </section>
      <section className='cupstats'>
        <h2 className="gold3">Team Statistics 2025 Copa De Primera </h2>
        <p>Matches Played:16 </p>
        <p>Goals Scored: 39 </p>
        <p>Assists: 26 </p>
        <p>Top Scorer: Pardo, 12 Goals </p>
        <p>Top Assister: Duarte, Pardo, 6 Assists</p>
        <p>POTY: Pardo </p>
        <p>YPOTY: Pardo</p>
        <p>Playoffs: Final</p>
      </section>
      <section className='libstats'>
        <h2 className="gold3">Team Statistics 2025 Copa Libertadores </h2>
        <p>Matches Played:13 </p>
        <p>Goals Scored: 41 </p>
        <p>Assists: 32 </p>
        <p>Top Scorer: Pardo, 15 Goals </p>
        <p>Top Assister: Pardo, 7 Assists</p>
        <p>POTY: Pardo </p>
        <p>YPOTY: Pardo</p>
        <p>Playoffs: Won</p>
      </section>
    </div>
  );
};

export default AboutPlayersPage;
